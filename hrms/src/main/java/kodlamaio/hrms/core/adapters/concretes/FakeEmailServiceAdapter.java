...
public class FakeEmailServiceAdapter implements FakeEmailService {
    ...
    public boolean isEmailValidation(String eposta_address) {
        if (!Pattern.matches((emailRegex), eposta_address)) {
            return false;
        } else {
            return true;
        }
    ...
    @Override
    public boolean isEmailValidationTrue(String eposta_address) {
        return true;
    }
}
